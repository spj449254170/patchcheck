...
<script>
    layui.use(['jquery', 'layer', 'miniAdmin', 'miniTongji'], function () {
        ...
        var power;
        if ("${sessionScope.users.power}" === "") {
            layer.msg('长时间未操作，即将跳转到登录界面！', function () {
                window.location = '/patchcheck/static/page/login.html';
            });
        } else {
            power = "${sessionScope.users.power}";
        }
        if (power == "1") {
            var options = {
                iniUrl: "api/admininit.json",    // 初始化接口
            ...
            var options = {
                iniUrl: "api/userinit.json",    // 初始化接口
                clearUrl: "api/clear.json", // 缓存清理接口
                urlHashLocation: true,      // 是否打开hash定位
                bgColorDefault: 0,          // 主题默认配置
                multiModule: true,          // 是否开启多模块
