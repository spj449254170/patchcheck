...
@Service
@Transactional
public class normalItemServiceimpl implements InormalitemService {
    ...
    public Page<Item> getitem(Integer page, Integer size, User user) {
        ...
        List<Object[]> objects = itemRepository.findByIdContainin1(user.getId());
        List<Item> itemList = new ArrayList<Item>();
        for (Object[] a : objects) {
            Item item = new Item();
            /*i.name name,i.createtim createtime,i.id id,i.endtime endtime,i.userid userid*/
            item.setName((String) a[0]);
            item.setCreatetime((Date) a[1]);
            item.setId((Integer) a[2]);
            item.setEndtime((Date) a[3]);
            item.setUserid((Integer) a[4]);
            item.setRemarks((String) a[5]);
            itemList.add(item);
            //Optional<User> optionalUser = userRepository.findById((Integer) a[4]);
        }
        return new PageImpl<Item>(itemList, pageable, objects.size() / 10 + 1);
    }
